      ...
      <section className="workspace">
        ...
        {view === 'guide' && (
          <div className="guide-view">
            <nav className="guide-topic-nav" aria-label="運用マニュアルの目次">
              <a href="#guide-flow-howto">フローチャートの作り方</a>
              <a href="#guide-chapters-howto">チャプター手順の活用方法</a>
            </nav>
            ...
            <section className="guide-section guide-howto" id="guide-flow-howto" aria-labelledby="guide-flow-howto-title">
              <header>
                <p className="eyebrow">作り方 1</p>
                <h2 id="guide-flow-howto-title">フローチャートを作る</h2>
                <p>「何をするか」と「結果によって次に何をするか」を、カードと矢印で表します。通常作業にも、異常時の分岐にも使えます。</p>
              </header>
              <ol className="guide-instructions">
                <li>
                  <h3>基本情報を入力し、白紙のフローにカードを置く</h3>
                  <p>「新規手順書」で作業名・整理No・品名・部署を登録します。「フローチャートで作成」の上部にある「作業」「判断」「完了」を押して、必要なカードを追加します。白紙部分を右クリックしてカードを置くこともできます。</p>
                  <p><strong>作業:</strong> 実施すること（例: 外観を確認する）。<strong>判断:</strong> 進み先を選ぶ条件（例: 判定結果は？）。<strong>完了:</strong> その経路の終わり（例: 確認完了）。</p>
                </li>
                <li>
                  <h3>カードを1回クリックして内容を編集する</h3>
                  <p>「移動」モードでカードをクリックすると編集画面が開きます。「表示内容」には短い作業名、「現場への指示」には実施方法・注意点・確認事項を入力し、「確定」を押します。未確定のテキスト変更を取り消すときは「戻す」を押します。</p>
                  <p>写真や動画は「資料追加」から登録します。添付済みのカードには添付マークが付きます。カードのドラッグは位置の移動で、編集画面は開きません。</p>
                </li>
                <li>
                  <h3>開始地点を決め、カードを順番につなぐ</h3>
                  <p>最初に表示したいカードで「開始地点に設定」を選びます。「つなぐ」を押し、<strong>接続元のカード → 接続先のカード</strong>の順にクリックすると矢印が付きます。続けてつなぐ場合も、毎回この2枚を順に選びます。</p>
                  <p>作業カードの通常の進み先は1つです。複数の進み先を選ばせる場所には判断カードを使い、各経路の最後に完了カードを置きます。</p>
                </li>
                <li>
                ...
                <li>
                  <h3>配置と接続を見直す</h3>
                  <p>「移動」でカードをドラッグし、空白部分をドラッグすると表示範囲をスクロールできます。矢印をクリックすると「接続先」の変更や「接続を削除」ができます。</p>
                  <p>「整える」で配置を調整し、意図と違った場合は「元に戻す」で戻します。カードの右クリックメニューから複製・削除もできます。</p>
                </li>
                <li>
                  <h3>保存して、すべての経路を試す</h3>
                  <p>画面上部の「手順書を保存」を押します。「フロー閲覧」で開始地点から選択肢を押し、写真・指示・進み先を確認します。1つの経路だけでなく、すべての分岐が完了まで進めるかを確認してから「公開に切替」を押してください。</p>
                </li>
              </ol>
              <div className="guide-worked-example">
                <h3>作成例: 外観検査の3択フロー</h3>
                <p>最初に「外観を確認する」（作業）を置き、「判定結果は？」（判断）につなぎます。その先に次の3経路を作ります。</p>
                <dl>
                  <div><dt>OK</dt><dd>検査結果を記録する（作業） → 確認完了（完了）</dd></div>
                  <div><dt>NG</dt><dd>社内の不適合品処置手順を確認する（作業） → 処置確認完了（完了）</dd></div>
                  <div><dt>判断できない</dt><dd>責任者へ確認する（作業） → 確認依頼完了（完了）</dd></div>
                </dl>
                <p>これは構成の例です。作業名・判定条件・処置内容は、実際の現場ルールに合わせてください。</p>
              </div>
            </section>

            <section className="guide-section guide-howto" id="guide-chapters-howto" aria-labelledby="guide-chapters-howto-title">
              <header>
                <p className="eyebrow">作り方 2</p>
                <h2 id="guide-chapters-howto-title">チャプター手順を活用する</h2>
                <p>動画の「どこで何をしているか」を時間ごとに登録する目次です。作業の実演を見せたいときや、必要な場面だけ繰り返し確認するときに使います。</p>
              </header>
              <ol className="guide-instructions">
                <li>
                  <h3>作業動画を登録して再生する</h3>
                  <p>作成画面に動画をアップロードし、手順の区切りとなる場面まで再生します。位置を正確に決めたい場合は、その場面で一時停止します。</p>
                </li>
                <li>
                  <h3>区切りの場面で「現在位置に手順追加」を押す</h3>
                  <p>「チャプター手順」の「現在位置に手順追加」を押すと、その時点の再生位置を「時間」に設定した手順が追加されます。再生中に押すこともできます。</p>
                  <p>「時間」は <code>00:15</code> のように分:秒で調整できます。動画内に存在する時刻を設定してください。動画をつなぎ直したりトリミングしたりした後は、各手順の開始位置も確認し直します。</p>
                </li>
                <li>
                  <h3>手順名と説明を入力する</h3>
                  <p>「手順名」には作業の見出し、「説明」には見る場所・動作のポイント・注意事項を書きます。1つの手順に内容を詰め込みすぎず、作業が切り替わるごとに追加します。</p>
                </li>
                <li>
                  <h3>その手順に必要な検査画像を追加する</h3>
                  <p>各手順の「この手順の検査画像」から「OK写真」「NG例写真」「判定基準」の画像を追加します。確認箇所には画像編集で矢印や図形を入れられます。</p>
                  <p>OK写真とNG例写真は「フラッシュテスト」の判定訓練にも利用します。テストの出題には、図形を入れる前の元画像を使います。</p>
                </li>
                <li>
                  <h3>動画を見ながら手順を確認する</h3>
                  <p>保存後に「閲覧」を開き、手順をクリックすると設定した開始位置へ移動します。動画と手順説明・検査画像を見比べ、時刻が正しいか確認してください。</p>
                </li>
                <li>
                  <h3>同じ作業をフローチャートでも使う</h3>
                  <p>手順の「フローへ追加」を押すと、<strong>手順名と説明</strong>を持つ作業カードが図に追加されます。追加したカードは「つなぐ」で前後のカードへ接続します。</p>
                  <p>手順名や説明を変更した後は「フローを更新」を押して、対応するカードへ再反映します。これは自動同期ではなく、チャプター手順からカードへの一方向の更新です。カード側で編集した手順名・説明も、この操作で置き換わります。</p>
                </li>
              </ol>
              <div className="guide-worked-example">
                <h3>登録例: 1分間の外観検査動画</h3>
                <div className="guide-example-table-wrap">
                  <table className="guide-example-table">
                    <caption>動画の区切りと説明の書き方</caption>
                    <thead><tr><th scope="col">時間</th><th scope="col">手順名</th><th scope="col">説明・添付資料の例</th></tr></thead>
                    <tbody>
                      <tr><td><code>00:00</code></td><td>検査前の確認</td><td>対象品と整理Noを確認する。確認箇所の写真を添付。</td></tr>
                      <tr><td><code>00:15</code></td><td>表面を確認</td><td>見る面と照明の向きを記載。OK・NG写真と判定基準画像を添付。</td></tr>
                      <tr><td><code>00:40</code></td><td>結果を記録</td><td>記録する項目と、判断に迷った場合の確認先を記載。</td></tr>
                    </tbody>
                  </table>
                </div>
              </div>
              <aside className="guide-transfer-note" aria-label="フローへ追加するときの注意">
                <strong>フローへ追加するときの注意</strong>
                <p>動画の開始時刻や検査画像は、作業カードにはコピーされません。フロー閲覧でも見せたい写真・動画は、カードの「資料追加」から別途登録してください。</p>
              </aside>
              <p className="guide-use-summary"><strong>使い分け:</strong> 作業の順番や判断による分岐はフローチャート、動作の実演や時間指定の見直しはチャプター手順。両方を作る場合は、動画で区切りを登録してから「フローへ追加」を使うと、手順名と説明の再入力を減らせます。</p>
            </section>
